...
void process_received_PDU(mic_tcp_pdu pdu, mic_tcp_sock_addr addr)
{
	...
	printf(__FUNCTION__); printf("\n");
	mic_tcp_pdu pdu_ack;
	pdu_ack.header.source_port = PORT_S;
	pdu_ack.header.dest_port = PORT_D;
	pdu_ack.header.seq_num = 0;
	pdu_ack.header.ack_num = 0;
	pdu_ack.header.syn = 0;
	pdu_ack.header.ack = 1;
	pdu_ack.header.fin = 0;
	pdu_ack.payload.data = 0;
	pdu_ack.payload.size = 0;
	if(pdu.header.seq_num == nb_attendu){
		printf("Paquet %d recu & stocké\n",nb_attendu);
		app_buffer_put(pdu.payload);
		pdu_ack.header.seq_num = nb_attendu;
		pdu_ack.header.ack = 1;
		nb_attendu++;
	}
